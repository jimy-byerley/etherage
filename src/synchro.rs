...
// ============================== Other function ==============================
    ...
    /// See for much detail
    async fn compute_delay(&mut self) -> Result<(), EthercatError<&'static str>> {
    ...
    async fn dc_offset_correction(&self) {
        // Disable Sync unit
        for slv in self.slaves.iter() {
            ...
            // Set DC correction
            //TODO compute offset
            self.master.fpwr(slv.index, dc::rcv_time_offset, slv.clock.system_time).await;
